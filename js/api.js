...
async function sendRequest(target, body) {
...
async function performRequest(action, { data, params, method }) {
  let url = getApiUrl();
  ...
  for (let attempt = 0; attempt < attempts; attempt += 1) {
    let response;
    try { response = await sendRequest(url, body); }
    catch (error) {
...
async function request(action, { data, params = {}, method = 'GET', fresh = false, ttl = CACHE_TTL } = {}) {
  ...
  const operation = performRequest(action, { data, params, method })
    .then(value => {
...
async function bootstrap(year = new Date().getFullYear()) {
  try { return await request('getBootstrap', { params:{ year } }); }
  catch (error) {
